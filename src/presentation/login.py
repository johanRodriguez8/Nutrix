...
class VentanaLogin(QMainWindow):
    ...
    def toggle_password_visibility(self, state):
        self.input_contraseña.setEchoMode(
    ...
    def validar_credenciales(self):
        ...
        password = self.input_contraseña.text()
        user = auth_service.authenticate(username, password)
        if user:
            self.abrir_ventana_principal(user.role)
        else:
            QMessageBox.warning(self, ERROR, TEXT_ERROR_LOGIN)

    def abrir_ventana_principal(self, rol: str):
        from src.presentation.main_window import VentanaPrincipal
