...
ASSIGN_COL = 7
ENABLE_COL = 8
class Robot1Window(QWidget):
    def __init__(self, coordinator:RobotCoordinator):
        super().__init__()
        self.coordinator = coordinator
        self.setWindowTitle("ROBOT 1 STATUS")
        self.showMaximized()

        layout = QVBoxLayout()

        titulo = QLabel("ROBOT 1 STATUS")
    ...
    def cargar_datos(self):
        ...
        cell_reassign = QWidget()

        lay_r = QHBoxLayout(cell_reassign)
        lay_r.setContentsMargins(16, 4, 16, 4)
        lay_r.setAlignment(Qt.AlignCenter)

        lay_r.addWidget(reassign_btn)

        self.tabla.setCellWidget(0, 8, cell_reassign)
...
class SubRobot1Window(QWidget):
    def __init__(self, coordinator:RobotCoordinator):
        super().__init__()
        layout = QVBoxLayout()
        self.coordinator = coordinator
        label = QLabel("WELCOME TO ROBOT 1 STATUS")
        label.setAlignment(Qt.AlignCenter)
        label.setStyleSheet("font-size: 30px; font-weight: bold; color: #2596be;")
        layout.addWidget(label)
        boton = QPushButton("ROBOT 1 STATUS")
        boton.setStyleSheet("""
                QPushButton {
    ...
    def abrir_status(self):
        ...
        self.win = Robot1Window(self.coordinator)
