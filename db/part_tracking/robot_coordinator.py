...
class RobotCoordinator(QObject):
    ...
    def runProgramToCompletion(self, part:Part):
        ...
        if self.loader.connected:
            ...
            while not self.robot.program_running and waitTime < TIME_OUT:
                waitTime =  time.time() - startTime
        ...
        try:
            while not self.stop:
                ...
                if self.currentPart != None:
                    ...
                    # if self.robotNum == robotToDebug:
                    #     print(f"R{self.robotNum}: PART IS NONE")
                    self.noPart.emit(self.robotNum)
                    time.sleep(10)
                if self.stop:
                    break
            self.dc.print(f"R{self.robotNum}: Cycle fully stopped", self.robotNum)
    ...
    def waitForEndHangerOk(self, program, part):
        ...
        # print(f"R{self.robotNum}: WILL START WAIT FOR END HANGER OK")
        # print(f"Hanger: {nextHanger}{conveyorEnd}")
        # print(f"R{self.robotNum}: IS OK: {isOk}")
        ...
        while not isOk:
            ...
            # if self.robotNum == robotToDebug:
            #     print(f"R{self.robotNum}: WAITING FOR END HANGER OK")
            #     print(f"Hanger num: {nextHanger} Conveyor:{conveyorEnd}")
            #     print(f"R{self.robotNum}: IS OK: {isOk}")
            if self.stop:
                break
            time.sleep(WAIT_UPDATE_TIME)
